...
- **Alpine.js `x-data` en `<tr>`**: Alpine.js no inicializa componentes correctamente cuando `x-data` está en un elemento `<tr>` (dentro de `<table>`). El botón aparece pero no colapsa. Solución: mover `x-data` a un `<div>` dentro de `<td>`. El texto estático dentro de `<span x-text="...">fallback</span>` se muestra sin Alpine; siempre incluir fallback visible para no dejar botones invisibles antes de que cargue Alpine.
- **yfinance campos porcentuales siempre en fracción decimal**: `dividendYield`, `returnOnEquity`, `profitMargins`, `revenueGrowth` se devuelven **siempre** como fracción decimal (0.034 = 3.4%), nunca como porcentaje. La lógica condicional `v * 100 if v < 1.0 else v` es incorrecta porque el caso `else` nunca debería existir. Siempre aplicar `v * 100` con cap sobre el valor fraccionario: `dividendYield` → cap `0.0 <= v <= 0.15`; `returnOnEquity` → cap `abs(v) <= 5.0`. Valores fuera del rango son datos corruptos de yfinance y deben descartarse (retornar `None`).
...
- **`_fig_to_response` usa `fig.savefig()`**: Se usa `fig.savefig()` en lugar de `plt.savefig()` para evitar race condition cuando requests concurrentes crean varias figuras; `plt.savefig()` guarda la figura "activa" en el estado global de matplotlib, que puede ser distinta a la figura generada para esta request. `plt.close(fig)` se llama en `finally` para garantizar liberación de memoria incluso si `savefig` falla.
- **`loading="lazy"` en imágenes ocultas**: No usar `loading="lazy"` en imágenes con `style="display:none"` (patrón shimmer) ni en imágenes dentro de secciones colapsables con `x-cloak/x-show`. En el primer caso el navegador nunca las carga (Intersection Observer no detecta display:none); en el segundo usar `:src="open ? '/chart/...' : null"` para cargar la imagen solo cuando se expande la sección. Alpine con `:src=null` elimina el atributo `src`, evitando requests innecesarios.
...
- **Monte Carlo refactorizado**: `_build_mc_params()` centraliza los cálculos (covarianza real, retornos multi-factor); `_run_mc_simulation()` ejecuta la simulación GBM vectorizada. `_get_mc_params_cached()` cachea los parámetros 5 min. `montecarlo_page` y `chart_montecarlo` consumen las mismas funciones compartidas. La covarianza usa 2 años de datos yfinance (`period="2y"`); los retornos esperados son idénticos al módulo de optimización (5 factores). σ_cartera = √(w′Σw) en lugar de suma ponderada de volatilidades individuales. 1 000 paths en lugar de 500. Etiquetas del gráfico usan `ax.get_xaxis_transform()` para no depender de ylim. `_mc_cache` se invalida junto con el resto de cachés en `_invalidate_csv_cache()`.
